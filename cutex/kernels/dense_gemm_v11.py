...
Fixed ``M=N=K=16384`` resource and launch accounting::
    ...
    TMEM / CTA (one participating SM's TMEM bank)
      ...
      layout footprint      = 256 accumulator + 16 SFA + 32 SFB = 304 columns

    Launch / B300 residency
      grid                  = (16384/128, 16384/256, 1) = (128,64,1)
                            = 8,192 CTAs = 4,096 two-CTA clusters
      ...
      resource upper bound  = one CTA/SM, hence 74 clusters at once and
                              ceil(4,096/74) = 56 cluster waves
...
# CUTLASS-style cluster block swizzle.  The physical launch remains a regular
# non-persistent grid, but its row-major cluster rank is decoded through a CuTe
# Layout whose first 64 ranks cover one 8 x 8 block of logical output tiles.
# This is the same layout algebra used by PersistentTileSchedulerParams for
# swizzle_size=8 and raster_along_m=True, without adding a persistent tile loop.
# The swizzle unit is a full (2,1,1) cluster, so both CTAs always stay paired.
...
def _dense_gemm_v11_kernel(
    ...
    cluster_tile_layout_mnl: cute.Layout,
    ...
    if is_iket_cluster:
        cute.experimental.iket.range_push("dense_gemm_v11")
        cute.experimental.iket.range_push("v11_prologue")

    # CuTe-layout thread-block swizzle.  For the fixed 64 x 64 cluster grid the
    # layout is (64,(8,8),1):(8,(1,512),4096).  Inverting its codomain rank with
    # get_flat_coord visits an 8 x 8 logical block before moving to the next N
    # block.  Both physical CTAs share cluster_linear, so the pair gets one
    # logical tile and still differs only in the two-CTA MMA V coordinate.
    #
    # Layout 代数：get_flat_coord 是 "offset -> coord" 的逆映射。
    # 正向 offset = 8*m + 1*n_inner + 512*n_outer，逆向就是混合基数分解：
    #   n_inner = linear % 8
    #   m       = (linear % 512) // 8
    #   n_outer = linear // 512
    # 于是发射顺序 rank 0..7 先扫 8 个 n_inner（m=0），rank 8..15 到 m=1，
    # 连续 512 个 rank 覆盖 64(m) x 8(n_inner) 的窄条，L2 里同行 A / 同列 B
    # 的复用被限制在 8 列宽的窗口内——这就是 CUTLASS swizzle_size=8 的效果，
    # 全部由一个 Layout 的求逆完成，不需要手写 Morton/除法解码。
    ...
    # fake_acc 只借它的 layout，真指针要等 tmem.allocate 之后回填。
    ...
    # 固定 swizzle 后的 (tile_m, tile_l)，把 4 维坐标张量降到 (bulk, k_tile)。
    # slice 是 layout composition 的特例：删掉被固定的模式，剩余模式 stride
    # 原样保留，所以 tma_g_a[(None, k_tile)] 直接指向第 k 个 K-tile 的 GMEM 坐标。
    ...
    if is_iket_cluster:
        cute.experimental.iket.range_pop()  # v11_prologue
    ...
    if warp == MMA_WARP:
        ...
        if is_leader_cta:
            ...
            # TMEM 里 acc / SFA / SFB 共享同一次 512 列分配，靠列偏移切开。
            # TMEM 的物理形状是 128 lane x 512 column，每格 32 bit；lane 对应
            # 本 CTA 的 M 行（128 行半块），column 是可分配资源：
            #
            #   column: 0                255 256  271 272   303 304    511
            #           +------------------+--------+---------+---------+
            #   lane 0  |                  |  SFA   |  SFB    |         |
            #    ...    |  acc  (FP32)     | (UE8M0 | (UE8M0  |  未使用  |
            #   lane127 |  128 x 256       |  x4/列) |  x4/列) |         |
            #           +------------------+--------+---------+---------+
            #              256 列            16 列     32 列     208 列
            #
            #   acc   : ((128,256),1,1)，一列放一个 FP32，N=256 -> 256 列。
            #   t_sfa : 每个 lane（M 行）在一个 K-stage 里需要 128/32 = 4 个
            #           scale，UE8M0 每列 32 bit 可打包 4 个 -> 数据本身只要
            #           1 列；扩到 16 列是 tcgen05 SF 格式要求的 4x 复制
            #           （SMEM atom 里 (4):(4) 那个重复模式）再乘 4 kblock。
            #   t_sfb : N=256 是 lane 数（128）的两倍，同样的结构对折进两组
            #           列，所以是 SFA 的 2 倍 = 32 列。
            #   合计 304 列 < 512 列（分配按整 bank 取 512，见模块 docstring）。
            #
            # find_tmem_tensor_col_offset 从 layout 的 codomain 大小算出列数,
            # recast_ptr 把 FP32 列地址重解释为 SF8——地址代数全在编译期完成。
            # （列数 256/16/32 与模块 docstring 的 304 列合计一致；SF 列内的
            #   精确 bit 排布由 make_tmem_layout_sfa/sfb 生成，本机不可 print
            #   验证，若需坐实可在设备上 cute.printf(t_sfa.layout)。）
            ...
            # s2t 拷贝的 layout 流程与 MMA partition 同构：
            # 1. filter_zeros 压掉 SF layout 的广播模式（stride=0），
            #    SMEM/TMEM 两侧都压成紧凑形状；
            # 2. make_s2t_copy 用 Cp4x32x128bOp 的 atom 形状对 TMEM 布局做
            #    tiled 除法，得到 "每条 tcgen05.cp 指令搬 4x32x128b" 的重复结构；
            # 3. partition_S 产出的是 SMEM descriptor 张量（同 r_a 的思路），
            #    partition_D 产出 TMEM 地址张量。
            # 之后 copy_sfa_src[(None,...,stage)] 的 stage 下标同样只是
            # descriptor 偏移 += stage * 512 B（SFA）/1 KiB（SFB）。
    ...
    if warp < MMA_WARP:
        ...
        # t2r 的 layout 除法：Ld32x32bOp.x128 一条指令让 32 个 lane 各读
        # 32-bit x 128 列。make_tmem_copy 用这个 atom 对 acc((128,256)) 做
        # tiled 除法 -> (atom 内, 行重复, 列重复)；get_slice(tid) 再固定
        # 128 个线程中的本线程，于是：
        #   t_acc : 本线程要读的 TMEM 片段
        #   t_out : 同一除法作用在 mma_g_c 上 -> 本线程负责的 GMEM C 元素，
        #           末三个模式仍是 (tile_m, tile_n, l)
        # 两侧经过同一除法，副本间逐元素对应，这就是 "partition 决定谁搬哪个
        # 元素" 的通用范式；r_acc/r_out 直接取 t_out 去掉 tile 模式后的 shape。
        ...
        # ---- 4 个 epilogue warp 如何分工写出本 CTA 的 128 x 256 acc ----
        # atom（一条 tcgen05.ld.32x32b.x128）读的是 32 lane x 128 列，
        # 恰好一个 warp（32 线程）一次发出。于是 acc(128,256) 被切成：
        #   M 方向 128/32 = 4 组，N 方向 256/128 = 2 组 -> 2 x 4 = 8 个
        #   32-lane x 128-col 的 atom-tile。
        # 8 个 tile 分给 4 个 warp，每个 warp 搬 2 个（循环/平铺由
        # get_slice(tid) 决定，即 CuTe tiled-copy 的线程映射——本机无法
        # print，精确的 warp->tile 线性顺序以设备上 t2r 的 value layout 为准）。
        # 一个 warp 内：第 l lane 读 TMEM 第 (该 tile 组首 + l) lane，
        # 横跨它被分配的 128 列 -> 每 lane 128 个 FP32，两条指令 256 个。
        # 一个线程整场搬 256 个 FP32，正好是 acc 自分配的那一行×256 列：
        #   4 warp x 32 lane x 256 列 = 128 x 256 = 32,768 元素 ✓
        # r_acc（F32）/r_out（BF16）取的就是这个块状，逐元素对应后
        # r_out.store(...) 写回 C 的对应行。分工的"行"由原子 32 lane 决定，
        # "列"由 128 列块决定：没有手工偏移，全是同一 tiled-copy 的除法。
    ...
    if warp < MMA_WARP:
        ...
        if is_iket_cluster:
            cute.experimental.iket.range_pop()
    if is_iket_cluster:
        cute.experimental.iket.range_pop()  # dense_gemm_v11
...
def dense_gemm_v11(
    ...
    # PersistentTileSchedulerParams(swizzle_size=8, raster_along_m=True)
    # constructs this cluster-level layout after dividing the CTA-tile problem
    # shape (128,64,1) by CLUSTER_SHAPE=(2,1,1):
    #
    #   shape  = (64, (8, 8), 1)
    #   stride = ( 8, (1,512),4096)
    #
    # Offset = 8*m + n_inner + 512*n_outer, so consecutive launch ranks cover
    # all n_inner values and then advance m; each 64-rank block is an 8 x 8
    # cluster tile.  Keeping this as a CuTe Layout lets get_flat_coord perform
    # the inverse mapping in device code without a hand-written Morton decoder.
    cluster_grid_m = M // MMA_TILE[0]
    cluster_grid_n = N // MMA_TILE[1]
    # M/N and the swizzle are specialization-time constants.  Mark this check
    # explicitly as a Python/JIT-time branch so ``raise`` is not staged as an
    # unsupported device-side early exit.
    if cutlass.const_expr(cluster_grid_n % CLUSTER_SWIZZLE_SIZE != 0):
        raise ValueError("cluster grid N must be divisible by the swizzle size")
    cluster_tile_layout_mnl = cute.make_layout(
        (
            cluster_grid_m,
            (CLUSTER_SWIZZLE_SIZE, cluster_grid_n // CLUSTER_SWIZZLE_SIZE),
            1,
        ),
        stride=(
            CLUSTER_SWIZZLE_SIZE,
            (1, CLUSTER_SWIZZLE_SIZE * cluster_grid_m),
            cluster_grid_m * cluster_grid_n,
        ),
    )
    # make_smem_layout_a
