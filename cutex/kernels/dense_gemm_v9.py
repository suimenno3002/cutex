...
def _dense_gemm_v9_kernel(
    ...
    if is_iket_cluster:
        cute.experimental.iket.range_push("dense_gemm_v9")
        cute.experimental.iket.range_push("v9_prologue")
    ...
    mma_tile_coord_mnl = (
        bid_m // cute.size(cluster_layout_vmnk, mode=[0]),
        bid_n,
        bid_l,
    )
    ...
    if is_iket_cluster:
        cute.experimental.iket.range_pop()  # v9_prologue
    ...
    if warp < MMA_WARP:
        ...
        if is_iket_cluster:
            cute.experimental.iket.range_pop()
    if is_iket_cluster:
        cute.experimental.iket.range_pop()  # dense_gemm_v9
