...
SOURCE_PATH = (
    Path(__file__).resolve().parents[1] / "cutex" / "kernels" / "dense_gemm_v9.py"
)
...
def test_v9_fixes_the_two_cta_tile_cluster_and_warp_roles():
    source = _source()
    ...
    assert "AB_STAGES, ACC_STAGES = 4, 1" in source
    assert "MMA_WARP = 4" in source
    assert "TMA_WARP = 5" in source
    assert "THREADS = 6 * 32" in source


def test_v9_host_side_uses_mxfp8_two_cta_and_a_companion_sfb_mma():
    source = _source()
...
def test_v9_builds_four_staged_tma_paths_without_pipeline_wrappers():
    source = _source()
...
def test_v9_expands_full_empty_mbarrier_protocol_and_two_cta_signaling():
    source = _source()
...
def test_v9_manually_tracks_ring_stage_phase_and_drains_tail():
    source = _source()
...
def test_v9_is_a_regular_grid_without_a_persistent_tile_scheduler():
    source = _source()
    tree = _tree()
...
def test_v9_exposes_role_and_stall_ranges_to_iket():
    source = _source()
...
def test_v9_preserves_the_five_pointer_launch_interface():
    function = _function("dense_gemm_v9")
