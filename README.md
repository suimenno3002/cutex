...
│       ├── dense_gemm_v9.py # 非 persistent 的 2SM + 手写 warp-specialized pipeline
...
`dense_gemm_v9.py` 是可运行的手写异步流水实现：固定 `CtaGroup.TWO`、`(2,1,1)` cluster、pair-wide `256 × 256 × 128` tile、当前四级 A/B/scale 环形缓冲和 `4 epilogue + 1 MMA + 1 TMA` warp 特化；常规 grid 中每对 CTA 只处理一个输出 tile，因此没有 persistent tile scheduler。device 侧直接使用 full/empty `mbarrier` 调度四路 TMA、2-CTA `tcgen05` MMA、TMEM 到寄存器的 BF16 epilogue，不依赖 `PipelineTmaUmma` 或 `PipelineUmmaAsync`。它已作为 `--implementation manual_pipeline_v9` 接入 `modal_dense_gemm.py`，并支持只对一个中部 cluster 写入细粒度 IKET ranges。
